 ...
 * */
static DM_Value eval_boolean_expression(DM_Boolean boolean_value)
{
    DM_Value   v;
...
*/
static void release_if_string(DM_Value *v)
{
    if (v->type == DM_STRING_VALUE) {
...
*/
static DM_Value eval_identifier_expression(DM_Interpreter *inter,
                           LocalEnvironment *env, Expression *expr)
{
    ...
    /*查找本地的变量*/
    vp = dm_search_local_variable(env, expr->u.identifier);
    if (vp != NULL) {
      /*找到*/
        v = vp->value;
    } else {
      /*如果没有找到，查找全局变量*/
        ...
        if (vp != NULL) {
          ...
          /*如果找不到，报错*/
            dm_runtime_error(expr->line_number, VARIABLE_NOT_FOUND_ERR,
 ...
 * */
static DM_Boolean eval_binary_boolean(DM_Interpreter *inter, ExpressionType operator,
                    DM_Boolean left, DM_Boolean right, int line_number)
{
    DM_Boolean result;
    ...
    if (operator == EQ_EXPRESSION) {
      ...
      /*不符合boolean之间的运算,报错*/
        ...
        dm_runtime_error(line_number, NOT_BOOLEAN_OPERATOR_ERR,
...
static void eval_binary_double(DM_Interpreter *inter, ExpressionType operator,
...
static DM_Boolean eval_compare_string(ExpressionType operator,
                    DM_Value *left, DM_Value *right, int line_number)
{
    DM_Boolean result;
    int cmp;
...
*/
static DM_Boolean eval_binary_null(DM_Interpreter *inter, ExpressionType operator,
                 DM_Value *left, DM_Value *right, int line_number)
{
    DM_Boolean result;

    if (operator == EQ_EXPRESSION) {
        result = left->type == DM_NULL_VALUE && right->type == DM_NULL_VALUE;
    } else if (operator == NE_EXPRESSION) {
        result =  !(left->type == DM_NULL_VALUE
                    && right->type == DM_NULL_VALUE);
    } else {
        char *op_str = dm_get_operator_string(operator);
        dm_runtime_error(line_number, NOT_NULL_OPERATOR_ERR,
                          STRING_MESSAGE_ARGUMENT, "operator", op_str,
                          MESSAGE_ARGUMENT_END);
    }
...
DM_String *
chain_string(DM_Interpreter *inter, DM_String *left, DM_String *right)
{
    int len;
    char *str;
    DM_String *ret;
    ...
    ret = dm_create_domi_string(inter, str);
    dm_release_string(left);
    dm_release_string(right);
 ...
 *
 * */
DM_Value dm_eval_binary_expression(DM_Interpreter *inter, LocalEnvironment *env, ExpressionType operator, Expression *left, Expression *right)
{
    DM_Value   left_val;
    DM_Value   right_val;
    DM_Value   result;
     ...
     * */
        ...
        eval_binary_double(inter, operator,
                           left_val.u.double_value, right_val.u.double_value,
                           &result, left->line_number);
    } else if (left_val.type == DM_BOOLEAN_VALUE
               && right_val.type == DM_BOOLEAN_VALUE) {
        result.type = DM_BOOLEAN_VALUE;
        ...
        if (right_val.type == DM_INT_VALUE) {
          ...
          /*将boolean类型的变量转换为true或者false*/
            if (right_val.u.boolean_value) {
                right_str = dm_create_domi_string(inter, MEM_strdup("true"));
            } else {
                right_str = dm_create_domi_string(inter, MEM_strdup("false"));
            }
          ...
          /*右边已经是string类型*/
            ...
            right_str = dm_create_domi_string(inter, MEM_strdup("null"));
        }
      ...
      /*当左边和右边都为string时,而且此时符号也不会为+*/
        ...
        result.u.boolean_value
            = eval_compare_string(operator, &left_val, &right_val,
                                  left->line_number);
    } else if (left_val.type == DM_NULL_VALUE
               || right_val.type == DM_NULL_VALUE) {
      /*当左右都为null*/
        result.type = DM_BOOLEAN_VALUE;
        /*判断等与不等*/
        result.u.boolean_value
       ...
       */
        char *op_str = dm_get_operator_string(operator);
        dm_runtime_error(left->line_number, BAD_OPERAND_TYPE_ERR,
                          STRING_MESSAGE_ARGUMENT, "operator", op_str,
                          MESSAGE_ARGUMENT_END);
    }
...
将逻辑表达式变为DM_Value
*/
static DM_Value eval_logical_and_or_expression(DM_Interpreter *inter,
                               ...
                               Expression *left, Expression *right)
{
    ...
    if (left_val.type != DM_BOOLEAN_VALUE) {
      /*左右必须为DM_Boolean*/
        dm_runtime_error(left->line_number, NOT_BOOLEAN_TYPE_ERR,
                          MESSAGE_ARGUMENT_END);
    }
     ...
     */
    right_val = eval_expression(inter, env, right);
    if (right_val.type != DM_BOOLEAN_VALUE) {
      /*右边必须为DM_Boolean*/
        dm_runtime_error(right->line_number, NOT_BOOLEAN_TYPE_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    /*
...
返回取负数之后的值
*/
DM_Value dm_eval_minus_expression(DM_Interpreter *inter, LocalEnvironment *env, Expression *operand)
{
...
static void dispose_local_environment(DM_Interpreter *inter, LocalEnvironment *env)
{
    while (env->variable) {
      /*回收局部变量*/
        Variable        *temp;
        temp = env->variable;
        if (env->variable->value.type == DM_STRING_VALUE) {
          /*因为string可能存在多个引用*/
            dm_release_string(env->variable->value.u.string_value);
        }
...
调用原生函数
*/
static DM_Value call_native_function(DM_Interpreter *inter, LocalEnvironment *env,
                     Expression *expr, DM_NativeFunctionProc *proc)
{
    DM_Value   value;
    int         arg_count;
    ArgumentList        *arg_p;
    ...
    /*分配空间*/
    args = MEM_malloc(sizeof(DM_Value) * arg_count);
                      ...
                      Expression *expr, FunctionDefinition *func)
{
    ...
    for (arg_p = expr->u.function_call_expression.argument,
             param_p = func->u.domi_f.parameter;
         arg_p;/*当没有实际参数时跳出循环*/
    ...
    if (param_p) {
      /*此时实际参数比形式参数少，报错*/
        dm_runtime_error(expr->line_number, ARGUMENT_TOO_FEW_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    result = dm_execute_statement_list(inter, local_env,
                                        func->u.domi_f.block
                                        ->statement_list);
...
在进行函数调用的表达式评估
*/
static DM_Value eval_function_call_expression(DM_Interpreter *inter, LocalEnvironment *env,
                              Expression *expr)
{
    DM_Value           value;
    ...
    if (func == NULL) {
      /*当找不到函数的时候*/
        dm_runtime_error(expr->line_number, FUNCTION_NOT_FOUND_ERR,
                          STRING_MESSAGE_ARGUMENT, "name", identifier,
                          MESSAGE_ARGUMENT_END);
    }
    switch (func->type) {
    case DOMI_FUNCTION_DEFINITION:
      /*调用自定义的函数*/
 ...
 *表达式评估，执行函数
 返回得到的DM_vALUE
 * */
static DM_Value eval_expression(DM_Interpreter *inter, LocalEnvironment *env, Expression *expr)
{
    DM_Value   v;
    switch (expr->type) {
    ...
    case LE_EXPRESSION:
        v = dm_eval_binary_expression(inter, env,
                                       expr->type,
    ...
    case MINUS_EXPRESSION:
        v = dm_eval_minus_expression(inter, env, expr->u.minus_expression);
        break;
...
DM_Value dm_eval_expression(DM_Interpreter *inter, LocalEnvironment *env,
                    Expression *expr)
{
